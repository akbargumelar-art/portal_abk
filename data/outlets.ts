...
const csvData = `CREATE_AT,OUTLET_ID,NAMA_OUTLET,KELURAHAN,KECAMATAN,KABUPATEN,CLUSTER,BRANCH,REGIONAL,AREA,LONGITUDE,LATTITUDE,NO_RS,NO_KONFIRMASI,KATEGORI,TIPE_OUTLET,FISIK,TIPE_LOKASI,KLASIFIKASI,SF_CODE,PJP,FLAG,TAP,Salesforce
...
function parseCSV(csv: string): OutletData[] {
  const lines = csv.split('\n').filter(line => line.trim() !== '');
  if (lines.length < 2) return [];

  const headers = lines[0].split(',').map(h => h.trim());
  
  const headerMap: { [key: string]: keyof OutletData } = {
    'CREATE_AT': 'createAt',
    'OUTLET_ID': 'outletId',
  ...
  return lines.slice(1).map(line => {
    ...
    const obj: any = {};
    camelCaseHeaders.forEach((key, index) => {
        obj[key] = values[index]?.trim() || '';
    });
    return obj as OutletData;
  });
}

export const outletData = parseCSV(csvData);