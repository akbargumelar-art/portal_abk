...
import { MENU_ITEMS, ADMIN_INPUT_MENU_ITEMS } from '../../constants';
...
const Sidebar: React.FC<SidebarProps> = ({ isCollapsed, onToggle }) => {
  ...
  const [floatingSubmenu, setFloatingSubmenu] = useState<{ top: number; item: MenuItem } | null>(null);
  const hideSubmenuTimeout = useRef<number | null>(null);

  const menuList = user?.role === UserRole.ADMIN_INPUT ? ADMIN_INPUT_MENU_ITEMS : MENU_ITEMS;
  ...
  useEffect(() => {
    const activeParent = menuList.find(item => 
  ...
  const accessibleMenuItems = menuList.map(item => {
    if (item.subItems) {
  ...
  const NavItem: React.FC<{ item: MenuItem }> = ({ item }) => {
    ...
    if (item.subItems && item.subItems.length > 0) {
      return (
        <div 
          className="relative"
          onMouseEnter={(e) => handleShowFloatingSubmenu(e, item)}
          onMouseLeave={handleHideFloatingSubmenu}
        >
          <button
          ...
          >
            <div className="flex items-center">
              <item.icon className={`h-6 w-6 shrink-0 ${hasActiveSubItem ? 'text-telkomsel-red' : 'text-telkomsel-gray-500'}`} />
  ...
  return (
    <>
      ...
      {floatingSubmenu && isCollapsed && (
        <div
            className="fixed top-0 bg-white shadow-lg rounded-md border py-1 w-52 z-50 animate-fade-in-down"
            style={{
                left: '5rem', // w-20
                top: `${floatingSubmenu.top}px`,
        ...
        >
            <div className="px-4 py-2">
                <span className="font-semibold text-sm text-telkomsel-gray-800">{floatingSubmenu.item.name}</span>
            </div>
            <ul className="border-t border-gray-200">
                {floatingSubmenu.item.subItems?.map(subItem => (
                  <li key={subItem.path}>
                    <NavLink
                      to={subItem.path}
                      onClick={handleHideFloatingSubmenu}
                      className={({ isActive }) =>
