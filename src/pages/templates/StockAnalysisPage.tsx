...
const StockAnalysisPage: React.FC<StockAnalysisPageProps> = ({ pageTitle, dataType, data }) => {
    ...
    const extendedFilteredDetailData = useMemo(() => {
        return data.filter((item: StockOutletDetail) => {
            ...
            // FIX: Added explicit Number() conversion and default value to fix arithmetic operation error.
            const totalStock = Number(item[dynamicKeys.stokAkhirOlimpiade] || 0) + Number(item[dynamicKeys.stokAkhirBeli] || 0);
            ...
            // FIX: Added explicit Number() conversion and default value to ensure robust arithmetic operations.
            const totalStokAkhir = Number(item[dynamicKeys.stokAkhirOlimpiade] || 0) + Number(item[dynamicKeys.stokAkhirBeli] || 0);
            // Fix: The guard in the filter ensures `item` is an object, fixing the spread operator error.
            return {
                ...item,
                flag: 'PJP FISIK',
                total_penjualan: Number(item[dynamicKeys.penjualanOlimpiade] || 0) + Number(item[dynamicKeys.penjualanBeli] || 0),
                total_redeem_or_so: Number(item[dynamicKeys.secondaryActionOlimpiade] || 0) + Number(item[dynamicKeys.secondaryActionBeli] || 0),
